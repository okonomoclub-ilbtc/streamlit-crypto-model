...
# 1. Obtención de datos enriquecidos
...
def get_enriched_data(symbol, start_date):
    ...
    # Limpieza de MultiIndex de yfinance
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)
    
    # Asegurar que Close sea un número
...
window_size = st.sidebar.slider("Días de historial", 30, 100, 60)
        ...
        with st.spinner('Entrenando Red Neuronal...'):
            ...
            with col2:
                ...
                for d, p in zip(future_dates, preds_flat):
                    ...
                    res_list.append({"Fecha": d.strftime('%Y-%m-%d'), "Precio": f"${p_val:,.2f}", "Tendencia": trend})
                    temp_last = p_val
...
st.caption("Desarrollado por: @Bookbinderr | App de Predicción LSTM")
